return {
  ...
  config = function()
    ...
    local img_prompt =
      "You should replace the image that you are sent, only following the instructions. Do not talk at all. Only output vaid code. Do not provide any backticks that surround the code. Never ever output backticks like this ```. Any comment that is asking you for something should be removed after you satisfy them. Other comments should left alone. Do not output backticks"
    ...
    local function make_anthropic_img_spec_curl_args(opts, prompt)
      local url = opts.url
        ...
        messages = {
          {
            role = "user",
            content = {
              { type = "text", text = "convert this image to react" },
      ...
      local curl_json_file = vim.fn.tempname() .. ".json"
      print(curl_json_file)
      local json_content = vim.json.encode(data)

      vim.fn.writefile(vim.split(json_content, "\n"), curl_json_file)

      local args = { "-N", "-X", "POST", "-H", "Content-Type: application/json", "-d", "@" .. curl_json_file }
    ...
    local function anthropic_img_replace()
      dingllm.invoke_llm_and_stream_into_editor({
        url = "https://api.anthropic.com/v1/messages",
        model = "claude-3-5-sonnet-20241022",
        api_key_name = "ANTHROPIC_API_KEY",
        system_prompt = system_prompt,
        replace = true,
      }, make_anthropic_img_spec_curl_args, dingllm.handle_anthropic_spec_data)
    end

    vim.keymap.set({ "n", "v" }, "<leader>K", anthropic_img_replace, { desc = "llm anthropic image" })
